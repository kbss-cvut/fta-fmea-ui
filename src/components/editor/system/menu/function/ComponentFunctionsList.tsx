...
const ComponentFunctionsList = ({ component }) => {
  ...
  const [functions, addFunction, , removeFunction, , allFunctions, generateFDTree, , , addExistingFunction] =
    useFunctions();
  ...
  const _handleCreateFunction = (values: any) => {
    let createFunction: Function = {
      name: values.name,
      requiredBehaviors: requiredFunctions,
    ...
    addFunction(createFunction).then((f) =>
      selectedFailureModes.forEach((fm) => addFailureModeToFunction(fm.iri, f.iri)),
    );
    reset(values);
  ...
  const updateFailureModes = (
    currentFailureModes: FailureMode[],
            ...
            {functions.map((f) => (
              <ListItem key={f.iri}>
                ...
                <ListItemSecondaryAction>
                  ...
                  <IconButton edge="end" aria-label="delete" onClick={() => handleDeleteFunction(f)} size="large">
                    <DeleteIcon />
                  </IconButton>
                </ListItemSecondaryAction>
              </ListItem>
              ...
              <FormGroup>
                ...
                <FormControl>
                  ...
                  <Box className={classes.button}>
                    <Button color="primary" variant="outlined" onClick={handleShowDialog} component="span">
                      Add existing
                    </Button>
            ...
            <Dialog open={dialog} onClose={handleShowDialog} fullWidth maxWidth="sm">
              ...
              <DialogContent>
                <SafeAutocomplete
                  ...
                  useSafeOptions={true}
                  options={allFunctions.filter((el) => !el.component || el.component.iri !== component.iri)}
              ...
              <DialogActions>
                <Button color="primary" onClick={closeDialogWindow}>
                  Cancel
                </Button>
                <Button color="primary" onClick={handleFunctionToAdd}>
                  {" "}
                  Add
                </Button>
              </DialogActions>
