...
import { Grid, Box, FormControl, FormGroup, IconButton, InputLabel, Select, TextField, MenuItem } from "@mui/material";
...
const ComponentFailureModesEdit = (props: {
  ...
  useEffect(() => {
    ...
    setFailureModeParts(props.selectedFailureMode.childBehaviors.map((fm) => allFailureModes.get(fm.iri)));

    setRequiredFailureModes(props.selectedFailureMode.requiredBehaviors.map((fm) => allFailureModes.get(fm.iri)));
