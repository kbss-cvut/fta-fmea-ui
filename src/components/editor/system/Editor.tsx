...
const Editor = ({ setAppBarName }: DashboardTitleProps) => {
  ...
  useEffect(() => {
    setMergeableComponents(
      sidebarSelectedComponent &&
        contextMenuSelectedComponent &&
        sidebarSelectedComponent.iri !== contextMenuSelectedComponent.iri,
    );
  }, [sidebarSelectedComponent, contextMenuSelectedComponent]);

  const mergeComponents = () => {
    if (
      contextMenuSelectedComponent.linkedComponent &&
  ...
  const merge = () => {
    componentService.mergeComponents(sidebarSelectedComponent.iri, contextMenuSelectedComponent.iri).then(() => {
      fetchSystem();
      setSidebarSelectedComponent(null);
    });
  };
