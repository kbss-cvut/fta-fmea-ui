...
const AppBar = ({ title, showBackButton = false }: Props) => {
    ...
    const renderMenu = (
        ...
        >
            {loggedUser.roles && loggedUser.roles.indexOf('ROLE_ADMIN') >= 0 && <MenuItem onClick={navigateToAdmin}>Administration</MenuItem>}
    ...
    return (
        <div>
            ...
            <ChangePasswordDialog open={changePasswordDialogOpen} handleCloseDialog={() => setChangePasswordDialogOpen(false)} user={getLoggedUser()} />
        </div>
    );
};
