...
export const schema = Yup.object().shape({
    severity: Yup.number()
        ...
        .max(10, 'Severity cannot be greater than 10').nullable(),
    occurrence: Yup.number()
        .transform(yupOptionalNumber)
        .min(1, 'Occurrence cannot be lower than 1')
        .max(10, 'Occurrence cannot be greater than 10').nullable(),
    detection: Yup.number()
        .transform(yupOptionalNumber)
        .min(1, 'Detection cannot be lower than 1')
        .max(10, 'Detection cannot be greater than 10').nullable(),
});