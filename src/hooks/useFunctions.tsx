...
export const FunctionsProvider = ({ children, componentUri }: FunctionProviderProps) => {
  ...
  const addExistingFunction = (functionToAdd: Function, component: Component) => {
    ...
    if (oldComponent != null) {
      componentService
        ...
        .then(() =>
          componentService.addFunctionByURI(componentUri, func.iri).then((f) => reassignVariables(f, component)),
        )
        .catch((reason) => showSnackbar(reason, SnackbarType.ERROR));
    } else {
      componentService
        .addFunctionByURI(componentUri, func.iri)
  ...
  const addRequiredFunction = async (functionUri: string, requiredFunctionUri: string) => {
    functionService
      .addRequiredFunction(functionUri, requiredFunctionUri)
      .catch((reason) => showSnackbar(reason, SnackbarType.ERROR));
  };

  const removeFunction = async (f: Function) => {
    componentService
      .removeFunction(componentUri, f.iri)
      .then((value) => {
  ...
  const generateFDTree = async (functionUri: string, systemName: string, functionName: string): Promise<FaultTree> => {
    ...
    return functionService
      .generateFDTree(functionUri, encodeURIComponent(systemName + "_" + componentName + "_" + functionName))
      .then((value) => {
  ...
  const getTransitiveClosure = async (functionUri: string, type: string) => {
    return functionService.getTransitiveClosure(functionUri, type).then((value) => value);
  };

  const getFailureModes = async (functionUri: string) => {
    return functionService
      .getImpairedBehavior(functionUri)
      .then((value) => {
        return value;
      })
      .catch((reason) => {
